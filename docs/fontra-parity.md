...
- 鋼筆在曲線上插點的參數 t 改用 `Bezier.project().t`（`PenTool.estimateSegmentT`），取代固定取樣。

移植的 `fit-cubic` 亦已用於畫筆：`src/font/fitCurve.ts` 在其上實作 Schneider 遞迴擬合，將 `BrushTool` 的手繪點流轉為平滑 cubic Bézier 鏈（非 fontra 直接移植，故未列入下表）。
...
編輯工具同理：fontra 的工具以 `views-editor/src/edit-tools-*.js` 實作，Kumiko 為 `features/editor/tools/`。行為（拖拉語意、量測數學）可參照移植，但 UI 接線（toolbar、快捷鍵、圖層註冊）為 Kumiko 自有。已對照實作的工具如下表；量測 / 幾何數學抽為 `src/font/` 下的純函式，與 fontra 的 hit-tester 行為對齊但非逐行移植。

| fontra 來源                                  | Kumiko 對應                                                                  | 備註                                                                                   |
| -------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `views-editor/src/edit-tools-power-ruler.js` | `features/editor/tools/PowerRulerTool.ts` + `sceneView/layers/powerRuler.ts` | 量測數學見 `src/font/powerRuler.ts`（純函式、單元測試）；以 sidebearing 線量字寬與邊距 |
