...
2. 驗證 `GSUB type 6 format 3` 的 generated FEA round-trip
   - 入口在 `gsubRuleParser.ts` / `gsubContextRuleParser.ts`。
   - coverage / backtrack / input / lookahead / lookup records 已能轉成 `ContextualRule`。
   - 下一步要用 fontTools compiler 或 HarfBuzz shaping regression 驗證 generated `.fea` 對真實字型的行為。
