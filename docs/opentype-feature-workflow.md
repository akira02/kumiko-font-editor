...
1. 加 Fira Code regression test
   - 使用 `test_glyphs/FiraCode-Regular.otf`。
   - 先鎖定目前 `calt` / `GSUB type 6 format 3` 會進 `unsupportedLookups` 的數量和 source section 狀態。
   - 測試目標不是一次全轉可編輯，而是確保每一步 parser 改動都不靜默丟資料。
2. 實作 `GSUB type 6 format 3` 讀取
   - 入口在 `gsubRuleParser.ts` / `gsubContextRuleParser.ts`。
   - 先把 coverage / backtrack / input / lookahead / lookup records 轉成可檢視的 `ContextualRule`。
   - 如果還不能安全編輯，也可以先標成 readonly imported lookup，但要比 `unsupportedLookups` 更有結構。
3. 擴充 raw `.fea` classifier
