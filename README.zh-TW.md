...
- **從任何地方開啟**：匯入本地 `.ufo`、`.designspace`、`.glyphs`、`.glyphspackage` 或 binary font 檔案，或直接從 GitHub repo 載入 UFO 專案（透過 Cloudflare Pages Functions 代理 archive 下載）。
...
- 錨定 UFO 模型（`groups.plist` / `kerning.plist`）的 kerning，匯出為 FEA；目前計畫見 [docs/kerning.md](docs/kerning.md)。
