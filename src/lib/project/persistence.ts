...
export const openDatabase = async () => {
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!database.objectStoreNames.contains(KUMIKO_PROJECTS_STORE)) {
        database.createObjectStore(KUMIKO_PROJECTS_STORE, {
          keyPath: 'projectId',
        })
      }
      if (!database.objectStoreNames.contains(KUMIKO_GLYPHS_STORE)) {
        const store = database.createObjectStore(KUMIKO_GLYPHS_STORE, {
        ...
        if (store && !store.indexNames.contains('byUnicode')) {
          store.createIndex('byUnicode', 'unicodes', {
            unique: false,
            multiEntry: true,
          })
      ...
      if (!database.objectStoreNames.contains(KUMIKO_UI_STATE_STORE)) {
