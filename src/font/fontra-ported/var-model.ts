...
// source: src-js/fontra-core/src/var-model.js
// Partial port of fontTools.varLib.models.VariationModel.
// The unused `sorted()` helper from upstream is intentionally dropped to
// satisfy noUnusedLocals. Keep in sync — see docs/fontra-parity.md

import { VariationError } from './errors'
import { isSuperset } from './set-ops'
import { clamp, reversedEnumerate, round } from './utils'
import { addItemwise, mulScalar, subItemwise } from './var-funcs'
...
export class VariationModel {
  ...
  constructor(locations: any[], axisOrder: string[] | null = null) {
  ...
  _computeMasterSupports() {
    ...
    for (let i = 0; i < regions.length; i++) {
      ...
      for (let j = 0; j < i; j++) {
        ...
        let relevant = true
        for (const [axis, [lower, peak, upper]] of Object.entries(region) as [
          string,
  ...
  _locationsToRegions() {
    ...
    for (const l of locations) {
      for (const [k, v] of Object.entries(l) as [string, number][]) {
    ...
    for (const loc of locations) {
      const region: Record<string, [number, number, number]> = {}
      for (const [axis, locV] of Object.entries(loc) as [string, number][]) {
        if (locV > 0) {
  ...
  interpolateFromDeltas(loc: any, deltas: any[]) {
...
function sortedLocations(locations: any[], axisOrder: string[] | null = null) {
...
function getDecoratedMasterLocations(locations: any[], axisOrder: string[]) {
  ...
  for (let i = 0; i < locations.length; i++) {
    ...
    const locEntries = Object.entries(loc) as [string, number][]
...
function locationsContainsBaseMaster(locations: any[]) {
  for (let i = 0; i < locations.length; i++) {
...
function objectGet(o: Record<string, number>, k: string, dflt: number) {
  const result = o[k]
  if (result === undefined) {
    return dflt
  }
...
export function locationToString(loc: Record<string, number>) {
  // Return a deterministic and unique *identifier* for a location.
  const sortedLoc: Record<string, number> = {}
...
export function locationToName(
...
export function unnormalizeValue(
  v: number,
  lower: number,
  dflt: number,
  upper: number
...
export function normalizeLocationSparse(
  location: Record<string, number>,
  axisList: any[]
) {
  // Normalizes location based on axis min/default/max values from axes.
...
export function unnormalizeLocation(
  location: Record<string, number>,
  axisList: any[]
) {
...
export function supportScalar(location: any, support: any, ot = true) {
  // Returns the scalar multiplier at location, for a master
  // with support. If ot is True, then a peak value of zero
  // for support of an axis means "axis does not participate". That
  // is how OpenType Variation Font technology works.
  let scalar = 1.0
  for (const [axis, [lower, peak, upper]] of Object.entries(support) as [
    string,
    [number, number, number],
  ][]) {
    let v
...
function interpolateFromDeltasAndScalars(deltas: any[], scalars: number[]) {
...
export function deepCompare(a: any, b: any): number {
...
export function mapForward(location: Record<string, number>, axes: any[]) {
  return _mapSpace(location, axes, _fromEntries)
}

export function mapBackward(location: Record<string, number>, axes: any[]) {
...
function _mapSpace(
  location: Record<string, number>,
  axes: any[],
  mapFunc: (mappingArray: any) => Record<number, number> | undefined
) {
  const mappedLocation: Record<string, number> = { ...location }
  const axesWithMap: Record<string, any> = {}
...
function _fromEntries(mappingArray: [number, number][]) {
  if (!mappingArray) {
    return undefined
  }
...
function _reverseFromEntries(mappingArray: [number, number][]) {
  if (!mappingArray) {
    return undefined
  }
...
export function makeSparseLocation(
  location: Record<string, number>,
  axisList: any[]
) {
...
export function makeSparseNormalizedLocation(location: Record<string, number>) {
  // location must be normalized
  const sparseLocation: Record<string, number> = {}
...
export function mapAxesFromUserSpaceToSourceSpace(axes: any[]) {
  return axes.map((axis) => {
    ...
    if (axis.mapping) {
      ...
      if (axis.values) {
        newAxis.values = axis.values.map((value: number) =>
...
export function isLocationAtDefault(
  location: Record<string, number>,
  axes: any[]
) {
  for (const axis of axes) {
...
export function makeDefaultLocation(axes: any[]) {
