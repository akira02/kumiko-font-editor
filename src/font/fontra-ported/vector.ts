...
// source: src-js/fontra-core/src/vector.js
...
export function subVectors(vectorA: Vector, vectorB: Vector) {
  return { x: vectorA.x - vectorB.x, y: vectorA.y - vectorB.y }
}

export function mulVectorScalar(vector: Vector, scalar: number) {
  return { x: vector.x * scalar, y: vector.y * scalar }
}

export function mulVectorVector(vectorA: Vector, vectorB: Vector) {
  return { x: vectorA.x * vectorB.x, y: vectorA.y * vectorB.y }
}

export function rotateVector90CW(vector: Vector) {
  return { x: vector.y, y: -vector.x }
}

export function vectorLength(vector: Vector) {
...
export function intersect(pt1: Vector, pt2: Vector, pt3: Vector, pt4: Vector) {
  ...
  // the input lines. Returns undefined if there is no intersection.
  let intersection
  ...
  if (Math.abs(determinant) > _EPSILON) {
    ...
    intersection = addVectors(mulVectorScalar(delta1, t1), pt1) as Vector & {
      t1: number
      t2: number
    }
    intersection.t1 = t1
...
export function distance(pt1: Vector, pt2: Vector) {
  return vectorLength(subVectors(pt2, pt1))
}

export function dotVector(vectorA: Vector, vectorB: Vector) {
